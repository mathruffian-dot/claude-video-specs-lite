...
> ⚙️ **這是研習簡易版（fork 自 [claude-video-specs](https://github.com/mathruffian-dot/claude-video-specs)）。**
> **唯一差別：移除所有需要付費 API Key 的功能**（OpenAI `draw` 生圖技能、OpenAI / ElevenLabs 付費 TTS）。
> 圖片改用**免金鑰的 Unsplash 直連**，旁白用**免費的 Edge-TTS**。
> Playwright / ffmpeg / Node.js / 字體等元件**全部保留**，完整影片渲染流程不變。
> 適合研習現場：只要內建工具就能跑完，不必申請任何 AI API 金鑰。
